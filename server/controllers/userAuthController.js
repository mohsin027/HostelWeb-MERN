...
export async function resendEmail(req, res) {
  ...
  try {
    ...
    const verifiedTempToken = jwt.verify(tempUserToken, process.env.JWT_SECRET_KEY);
...
export async function userRegisterVerify(req, res) {
  try {
    ...
    const tempUserToken = req.cookies.tempUserToken;
    console.log(tempUserToken, "temptoken user register");
    if (!tempUserToken) {
...
export const userLogin= async(req,res)=>{
  ...
  try {
    ...
    const user = await UserModel.findOne({email})
    console.log(user,'user login cntrl');
    if(!user){
...
export const checkUserLoggedIn = async (req, res) => {
  try {
      ...
      console.log(user.hostelData,'455555555');
      if (!user) {
