import { MDBBtn, MDBCard, MDBCardBody, MDBCardHeader, MDBCardTitle, MDBDropdown, MDBDropdownItem, MDBDropdownMenu, MDBDropdownToggle, MDBTable, MDBTableBody, MDBTableHead } from 'mdb-react-ui-kit'
...
//   const { hostel } = useSelector((state) => state.auth.hostel);
  ...
  const handleStatus = async (status, id, index) => {
    ...
    try {
      const { data } = await axios.patch("/admin/hostel/complaintStatus", {
        stat: status,
        id,
      });
      if (!data.error) {
            ...
            <MDBTableBody>
              {
                ...
                <tr key={complaint._id}>
                  <td>
                    {moment(complaint.createdAt).format("YYYY-MM-DD")}
                  </td>
                  <td>
