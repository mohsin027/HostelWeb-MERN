...
const LinkItems = [
  ...
  { name: 'Complaints', link:'/admin/complaints',icon: FiCompass },

]

export default function Sidebar() {
  const { isOpen, onOpen, onClose } = useDisclosure()
  const {adminSidebarOpen} = useSelector((state)=>state.common)
  ...
  return (
    <Box minH="100vh" bg={useColorModeValue('gray.100', 'gray.900')}>
      ...
      <SidebarContent onClose={ ()=>dispatch({type:'TOGGLE-ADMIN-SIDEBAR'})} display={{ base: 'none', md: 'block' }} />
      <Drawer
        isOpen={adminSidebarOpen}
        placement="left"
        onClose={()=>dispatch({type:'TOGGLE-ADMIN-SIDEBAR'})}
        returnFocusOnClose={false}
        onOverlayClick={()=>dispatch({type:'TOGGLE-ADMIN-SIDEBAR'})}
        size="full">
        <DrawerContent>
          <SidebarContent onClose={()=>dispatch({type:'TOGGLE-ADMIN-SIDEBAR'})} />
        </DrawerContent>
      </Drawer>
      {/* mobilenav */}
